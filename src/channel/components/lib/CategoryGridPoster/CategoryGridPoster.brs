...
Function onContentChange()
  ...
  if m.top.itemContent <> invalid then
    ...
    if categoryContent <> invalid and m.top.itemContent.gridItemType <> invalid then
      if m.top.itemContent.gridItemType = m.gridItemTypes.linear and getExperimentResource("roku_linear_epg", "roku_linear_epg_v2", false).update_homescreen = true
        setUpLinear()
      else
...
Function isLinear(itemContent, gridItemTypes)
  bIsLinear = false
  if itemContent.gridItemType = gridItemTypes.linear and getExperimentResource("roku_linear_epg", "roku_linear_epg_v2", false).update_homescreen = true
    bIsLinear = true
  end if
