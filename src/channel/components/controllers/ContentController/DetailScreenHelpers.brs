...
Function showDetailScreen(content, sendTrackingOnResponse = true, successCb = invalid, errorCb = invalid, playbackSource = { "srcForAnalytic": "unknown", "srcForAds": "unknown" }) as Void
  experiment = getStatsigExperimentResource("", "roku_content_details_v7", true)
...
Function onRefreshRelatedContentSignal(msg) as Void
  if m.ymalDisplay = "hidden" then return
  detailScreen = msg.getRoSGNode()
...
Function handleSingleContentError(error, onRetrySuccessCallback, onRetryErrorCallback)
  ...
  if detailScreen <> invalid AND m.isScreenLoaded = false
    ...
    ' Remove pageOriginDetails once we figure out the root cause of series invalid for component interaction events.
    pageOriginDetails = {
      "pageOrigin": m.constants.ui.screenIds.detailScreen
      "functionName": "handleSingleContentError"
    }

    pageOriginDetails.previousScreen = m._.get(getHiddenScreen(1), "id")
    pageOriginDetails.errorCode = error.code
    pageOriginDetails.errorResponseContentId = error.contentId
    detailScreen.pageOriginDetails = pageOriginDetails

    populateDetailScreen(detailScreen, detailScreen.content, false, -1)
  end if
...
Function onDetailBackButtonPressedChange(msg = invalid)
  ...
  if msg <> invalid AND detailScreen <> invalid AND detailScreen.trackingPageInfo <> invalid
    ...
    if isNonEmptyString(lastKey) = true
      sendButtonComponentInteractionEvent(lastKey, "IMAGE", "CONFIRM", detailScreen)
    end if
  end if
...
Function skipDetailScreen(refreshedContent) as Void
  ...
  if detailScreen <> invalid
    ...
    if refreshedContent.needsLogin = false AND detailScreen.availabilityType <> m.constants.ui.contentTimings.upcoming
      ...
      ' TODO: Re-visit this if we do not graduate roku_content_details_v7 experiment.
