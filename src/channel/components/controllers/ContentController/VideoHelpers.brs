...
Function setupVideoPlayer(content, playbackSource = {"srcForAnalytic": "unknown", "srcForAds": "unknown"}, position = 0)
  ...
  videoPlayer.autoPlayLikeDislikeState = sLikedState
...
Function returnToDetailScreenFromVideo(sendAnalyticsEvent = true, shouldUpdateEpisodeScreenContent = true)
  ...
  if shouldShowContinueWatchingConsentDialog = true AND m.wasUserShownContinueWatchingConsentDialog = false AND isLoggedInUser() = true AND getConsentOptOutStatusByKey(m.constants.consentKeys.continueWatching) = true
    ...
    if isDeviceInUS() = true AND isUserAllowedToManageConsent() = true AND getExperimentResource("roku_cw_consent_existing_user", "roku_cw_consent_existing_user_after_plyback_v3", true).enabled = true
...
Function onAutoPlayLikeRemoveLikeSelected(msg)
  tubiLog("VideoHelpers.onAutoPlayLikeRemoveLikeSelected")
  videoPlayer = msg.getRoSGNode()

  autoPlayLikeDislikeState = videoPlayer.autoPlayLikeDislikeState

  if isLoggedInUser() = true
    if autoPlayLikeDislikeState = m.constants.ui.likeDislikeStates.liked
      autoPlayLikeDislikeState = m.constants.ui.likeDislikeActions.removeLike
    else
      autoPlayLikeDislikeState = m.constants.ui.likeDislikeActions.like
    end if

    updateLikeDislikePlayer(videoPlayer, autoPlayLikeDislikeState)
  else
    autoPlayLikeDislikeState = m.constants.ui.likeDislikeActions.like
    likeDislikeParams = {likeDislikeState:autoPlayLikeDislikeState, menuItem: "LikeMenuItem" }
...
Function onAutoPlayDisLikeRemoveDislikeSelected(msg)
  tubiLog("VideoHelpers.onAutoPlayDisLikeRemoveDislikeSelected")
  videoPlayer = msg.getRoSGNode()

  autoPlayLikeDislikeState = videoPlayer.autoPlayLikeDislikeState

  if isLoggedInUser() = true
    if autoPlayLikeDislikeState = m.constants.ui.likeDislikeStates.disliked
      autoPlayLikeDislikeState = m.constants.ui.likeDislikeActions.removeDislike
    else
...
Function sendAutoPlayLikeDislikeSelectedAnalytics(playerScreen, sRatingValue)
  trackingPageInfo = playerScreen.trackingPageInfo

  '//Send Analytics of user interaction with the like/dislike button
  sAnalyticsEventType = ""
  if sRatingValue = m.constants.ui.likeDislikeActions.like
    sAnalyticsEventType = "LIKE"
  else if sRatingValue = m.constants.ui.likeDislikeActions.dislike
    sAnalyticsEventType = "DISLIKE"
  else if sRatingValue = m.constants.ui.likeDislikeActions.removeLike
    sAnalyticsEventType = "UNDO_LIKE"
  else if sRatingValue = m.constants.ui.likeDislikeActions.removeDislike
...
Function sendExplicitFeedBackEvent(screen, sLikeEventEnum)
  ...
  sAnalyticsEventType = ""
    if sLikeEventEnum = m.constants.ui.likeDislikeActions.like
      sAnalyticsEventType = "LIKE"
    else if sLikeEventEnum = m.constants.ui.likeDislikeActions.dislike
      sAnalyticsEventType = "DISLIKE"
    else if sLikeEventEnum = m.constants.ui.likeDislikeActions.removeLike
      sAnalyticsEventType = "UNDO_LIKE"
    else if sLikeEventEnum = m.constants.ui.likeDislikeActions.removeDislike
      sAnalyticsEventType = "UNDO_DISLIKE"
    end if
  ...
  targetOneof.content.user_interaction = sAnalyticsEventType
...
Function updateLikeDislikePlayer(playerScreen, sRatingChange)
  if playerScreen <> invalid

    updateLikeDislikeRequestInfo = m.userDeviceApi.setContentRating(playerScreen.content.id, sRatingChange)
    ...
    m.makeRequest({
      ...
      successCallback: onAutoPlayLikeChangedSuccess
      errorCallback: onAutoPlayLIkeChangedError
...
Function onAutoPlayLikeChangedSuccess(response)
  tubiLog("VideoHelpers.onAutoPlayLikeChangedSuccess")
  videoPlayerScreen = getFromScreenCache(m.constants.ui.screenIds.videoPlayerScreen)
  if response <> invalid AND type(response.data) = "roArray"
    returnedContentId = response.data[0]
    sReturnedAction = response.action
    m.Bookmarks.updateLikesLocally(returnedContentId, sReturnedAction, m.global)

    '//Tell Video Player Screen how to react to user interacting with the like/dislike button
    if videoPlayerScreen <> invalid AND videoPlayerScreen.content <> invalid
      if videoPlayerScreen.content.id <> invalid AND returnedContentId <> invalid AND returnedContentId = videoPlayerScreen.content.id
        ...
        sLikedState = translateLikeActionToLikeState(sReturnedAction)
        if sLikedState =  m.constants.ui.likeDislikeStates.liked
        ...
        else if sLikedState =  m.constants.ui.likeDislikeStates.disliked
...
Function onAutoPlayLIkeChangedError(parsedReturn)
  tubiLog("VideoHelpers.onAutoPlayLIkeChangedError")
  ...
  if videoPlayerScreen <> invalid AND videoPlayerScreen.content <> invalid
    ...
    currentScreen = getCurrentScreen()
    if currentScreen <> invalid AND currentScreen.id = videoPlayerScreen.id

      '//set back to the previous like state before the failed attempt
      sPreviousLikeState = getLike(videoPlayerScreen.id)
      sLikedState = translateLikeActionToLikeState(sPreviousLikeState)
      videoPlayerScreen.autoPlayLikeDislikeState = sLikedState

      '//Make sure the current screen is associated with the detail screen that caused the error
      if code <> invalid AND videoPlayerScreen.content.id <> invalid AND returnedContentId <> invalid AND returnedContentId = videoPlayerScreen.content.id
        sErrorSubtype = ""

        ' set up the error modal dialog
        errorCode = getUserFacingErrorCode(m.constants.errors.context.playerScreen, sErrorSubtype, code)
        ...
        showErrorModal(modalInfo, onLikeDislikeRetry, [videoPlayerScreen, returnedAction])
      end if
