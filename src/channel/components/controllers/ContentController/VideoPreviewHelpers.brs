...
Function stopVideoPreview(node = invalid)
  ...
  if node <> invalid AND (node.subType() = "VideoPreviewPlayer" OR node.subType() = "ReusablePreviewPlayer") AND isListScrolling = false
...
Function onVideoPreviewStateChanged(msg)
  ...
  if videoPreviewState = "finished" AND currentScreen <> invalid
    ...
    if item <> invalid
      isReplay = false
      if item <> invalid AND (item.gridItemType = m.constants.ui.gridItemTypes.skinAd OR item.gridItemType = m.constants.ui.gridItemTypes.adRowlistSpotlight)
        isReplay = true
      end if
