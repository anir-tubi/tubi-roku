...
Function showEPGScreen(constants, screenID = "", componentToFocus = "")
  ...
  if epgScreen <> invalid
    ...
    if m.pub_serverPersistentData.secondSessionLinearNotWatched = true AND getExperimentResource("roku_linear_epg_education_modal_over_homegrid", "roku_linear_epg_education_modal_over_homegrid_v2", false).enabled = true
...
Function onChannelLikeDislikeInfo(msg)
  favChannelInfoAA = msg.getData()
  updateLikeDislikeLinear(favChannelInfoAA)
End Function


'This function will handle the like/dislike for a channel when user clicks on channel logo
'@favChannelInfoAA = AA,  will contain
'                 {channelNode: node, liked/disliked channel
'                  action: enum m.constants.ui.likeDislikeActions
'                  }
Function updateLikeDislikeLinear(favChannelInfo)
  tubilog("EPGScreenHelpers.updateLikeDislikeLinear")

  stopCountdownTimer()

  if favChannelInfo <> invalid

    isSignedInUser = isLoggedInUser()
    channel = favChannelInfo.channelNode

    if isSignedInUser = true

      epgChannelList = getFromContentCache(m.constants.ui.contentIds.timeGridContent)

      if epgChannelList <> invalid

        if favChannelInfo.action = m.constants.ui.likeDislikeActions.like
          addFavoriteChannelToEPGContent(epgChannelList, favChannelInfo)
        else
          removeFavoriteChannelFromEPGContent(epgChannelList, favChannelInfo)
        end if

      end if

    else
      'stop the video player so that signIn screen can be shown.
      linearVideoPlayer = getFromScreenCache(m.constants.ui.screenIds.linearVideoPlayerScreen)
      if linearVideoPlayer <> invalid AND linearVideoPlayer.fullscreen = true
        stopLinearVideoContent() '//In case the video is still playing
        linearVideoPlayer.closeTransport = true
        linearVideoPlayer.waitingForRFI = true
      else
        stopAndHideLinearVideoPlayer()
      end if

      startSignIn(afterSignInLikeDislikeLinear, favChannelInfo)
      channel.selected = false 'it will be updated after signIn
    end if
  end if
...
Function onChannelLikeChangedSuccess(requestBody)
  tubiLog("EPGScreenHelpers.onChannelLikeChangedSuccess")

  if requestBody <> invalid AND type(requestBody.data) = "roArray"
    if requestBody.action = m.constants.ui.likeDislikeActions.like
      message = getTranslation("epg_favorites_toast_message")
...
' Delete this function when favorites container has been added to EPG response. and also container_id param  logic related to favorites from tubiAds.brs
Function addFavoritesToEPGResponse(response)
  linearLikes = getFieldFromGlobal("linearLikeIds")

  favorites = {}
  for each channel in response.getChildren(-1, 0)
    likedChannel = m.nodeHelpers.getChildById(linearLikes, channel.id)

    if likedChannel <> invalid
      channel.selected = true

      if favorites[channel.Id] = invalid
        favoriteChannel = channel.clone(true)
        favoriteChannel.parentId = m.constants.ui.categoryIds.favorites
        favoriteChannel.parentTitle = "Favorites"
        favorites[channel.Id] = favoriteChannel
      end if

    end if
  end for

  for each favorite in favorites.keys()
    response.insertChild(favorites[favorite], 0)
  end for
...
Function addFavoriteChannelToEPGContent(epgContent, favChannelInfo)
  tubilog("EPGScreenHelpers.addFavoriteChannelToEPGContent")
  channel = favChannelInfo.channelNode
  channelId = channel.Id

  if channelId <> invalid AND epgContent <> invalid

    likedChannel = getLinearLike(channelId)

    if likedChannel = invalid  AND channel <> invalid 'fav channel already not there
      ...
      ' Work Around:
      ' first replace the first child to be newly selected favorite channel
      ' then shift all the channels one position down
      ' epgContent.insertChild is simplest way to do it.But for some unknown reason it is taking time which is causing execution timeouts in lower end devices.

      epgAA = []
      ' Appending all the children to array.
      epgAA.append(epgContent.getChildren(-1, 0))

      lastItem = epgAA.peek()

      ' adding channel to the top of the array.
      epgAA.Unshift(favoriteChannel)

      ' Replacing all child nodes starting from zero.
      epgContent.replaceChildren(epgAA, 0)

      ' Since we have one additional channel added at top pushing the last channel back to the node.
      epgContent.appendChild(lastItem)

      updateAllChannelsForFavorites(epgContent, favChannelInfo, true)

      ' Update the like/dislike info to server for retention.
      updateLinearLikesToServer(favChannelInfo)

      ' Update the like/dislike info locally to be used within this session.
      m.Bookmarks.addLinearLikeLocally(channelId, m.global)

      sendLinearBookmarkAnalytics(channelId, "ADD_TO_QUEUE")
    end if
...
Function removeFavoriteChannelFromEPGContent(epgContent, favChannelInfo)
  tubilog("EPGScreenHelpers.removeFavoriteChannelFromEPGContent")
  channel = favChannelInfo.channelNode
  channelId = channel.Id
  ...
  if channelId <> invalid AND epgContent <> invalid
    ...
    updateAllChannelsForFavorites(epgContent, favChannelInfo, false)

    ' Update the like/dislike info to server for retention.
    updateLinearLikesToServer(favChannelInfo)

    ' Update the like/dislike info locally to be used within this session.
    m.Bookmarks.removeLinearLikeLocally(channelId, m.global)
    sendLinearBookmarkAnalytics(channelId, "REMOVE_FROM_QUEUE")
  end if

End Function


'Send like/dislike info to backend to keep track for next time
Function updateLinearLikesToServer(favChannelInfo)
  channel = favChannelInfo.channelNode
  updateLikeDislikeRequestInfo = m.userDeviceApi.setContentRating(channel.Id, favChannelInfo.action, "linear")
  ...
  m.makeRequest({
    ...
    successCallback: onChannelLikeChangedSuccess
    errorCallback: onChannelLikeChangedError
    responseType: "assocarray"
  })
  linearVideoPlayer = getFromScreenCache(m.constants.ui.screenIds.linearVideoPlayerScreen)

  if linearVideoPlayer <> invalid AND linearVideoPlayer.fullscreen = false AND linearVideoPlayer.state = "playing"
    startCountDowntimer()
  end if
...
' EPG has many duplicate channels and when a channel is liked/disliked,
' it's star display should change every where.
' since we are going over the channel list to change the star, we as well use this loop to jump to the
' correct channel after like/dislike  (like moves the focus to the previous item due to insertion and dislike moves the focus to the next item)
'
' @epgContent - node, entire timeGrid content
' @favChannelInfo - AA, will contain channel info of the channel that is favorited or unvorited
'                 {ChannelNode: node, like/disliked channel
'
'                  action: enum m.constants.ui.likeDislikeActions
'                  }
' @isSelected - boolean, favorite = true ; unfavorite = false
Function updateAllChannelsForFavorites(epgContent, favChannelInfo, isSelected)
  tubilog("EPGScreenHelpers.updateAllChannelsForFavorites")
  ...
  for i = 0 to epgContent.getChildCount() - 1
    ...
    if item <> invalid AND item.id = channelId
...
' Organizes the LINEAR information needed to create a "bookmark" tracking event AND sends the information to the trackingTask which will
' actually send the event.
'
' @contentId: favorited/unfavorited linear content Id
' @operation: string, one of the valid operations as defined in events.protos -> Bookmarks -> enum Operation
Function sendLinearBookmarkAnalytics(contentId, operation)
  ...
  if isAnEPGScreen(currentScreen) = true
  ...
  pageInfo = {
    pageType: pageType
    pageValues: {
      video_id: contentId.toInt()
    }
  }

  bookmarkAnalyticsEvent.pageOneof = m.tracking.getAnalyticsPage(pageInfo.pageType, pageInfo.pageValues)

  bookmarkAnalyticsEvent.contentOneof.video_id = contentId.toInt()
