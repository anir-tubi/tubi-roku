...
Function onIsLoading()
  ...
  if m.top.relatedContent <> invalid AND m.top.relatedContent.getChildCount() > 0
    m.RelatedContentParentGroup.visible = not m.top.isLoading
  else
    m.RelatedContentParentGroup.visible = false
  end if
...
Function focusMenu(immediately = false)
  ...
  if immediately
    ...
    m.Info.opacity = 1.0
    m.Menu.opacity = 1.0
    m.RelatedContentParentGroup.translation = [0, 921]
  else
    if m.top.isSeries = true
      ...
      slideTo(m.RelatedContentParentGroup, [0, 921], m.focusAnimationDuration)
      m.RelatedContentParentGroup.opacity = 0.2
...
Function focusRelated()
  ...
  if m.top.isSeries = true
    ...
    ' m.top.backgroundUriList = m.movieOverlay.backgroundUriList
...
Function onKeyEvent(key As String, press As Boolean) as Boolean
  ...
  if press then
    ...
    ' Down presses arrive here if not consumed by the menu, meaning it's already at the bottom button
      ...
      if m.Menu.isInFocusChain() = true AND m.RelatedContentParentGroup.visible = true then
        focusRelated()
        return true
      ...
      if m.SecondaryMenu.isInFocusChain() = false AND m.RelatedContentParentGroup.isInFocusChain() = false
        m.top.backButtonPressed = true
        return true
