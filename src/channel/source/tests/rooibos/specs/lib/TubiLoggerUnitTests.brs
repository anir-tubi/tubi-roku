...
Function tubiLogger_getLoggingRequest_test()
  ...
  m.assertEqual(logRequest.url, constants.urls.datascience.logging)
  m.assertTrue(type(logRequest.body) = "String")
  m.assertTrue(logRequest.body.len() > 0)
  logRequestBody = ParseJson(logRequest.body)
  m.assertNotInvalid(logRequestBody["app_id"])
  m.assertNotInvalid(logRequestBody.platform)
  m.assertNotInvalid(logRequestBody["device_id"])
  m.assertNotInvalid(logRequestBody.ua)
  m.assertNotInvalid(logRequestBody.version)
  m.assertNotInvalid(logRequestBody.message)
  m.assertNotInvalid(logRequestBody.level)
  m.assertNotInvalid(logRequestBody.subtype)
  m.assertNotInvalid(logRequestBody["type"])
