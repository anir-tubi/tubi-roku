...
Function tubiMetadataTranslate_translateEPGChannelIds_test()
  ...
  m.assertNotInvalid(translated)
  m.assertTrue(translated.getChildCount() = 248)
  singleChannel = translated.getChild(0)
  m.assertNotEmpty(singleChannel.id)
  m.assertTrue(singleChannel.id = "682633")
  m.assertTrue(singleChannel.parentTitle = "Featured")
  m.assertTrue(singleChannel.title = "FIFA World Cup")
  m.assertNotInvalid(type(singleChannel.videoResources) = "roArray")
  m.assertTrue(singleChannel.videoResources.count() > 0)
  m.assertNotInvalid(type(singleChannel.backgrounds) = "roArray")
  m.assertTrue(singleChannel.backgrounds.count() > 0)
  m.assertNotInvalid(type(singleChannel.SUBTITLETRACKS) = "roArray")
  m.assertTrue(singleChannel.SUBTITLETRACKS.count() > 0)
  m.assertTrue(singleChannel.hasVideoResources = true)
  m.assertTrue(singleChannel.hasSubtitles = true)
  m.assertNotInvalid(type(singleChannel.thumbnailUrls) = "roArray")
End Function
...
Function tubiMetadataTranslate_translateEPGPrograms_test()
  ...
  m.assertEqual(channelInfo.title, "fubo Sports Network")
