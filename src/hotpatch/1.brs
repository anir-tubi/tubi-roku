print "hot patch 1: force mp4 (unless specified).  Lower number of tracking messages.  Fix SD playback. threshold redirect screen"

san = m.app.settings.shortAppName
...
m.app.cp.urls = {
		  getPlaylists: "http://api.adrise.tv/v2/app.php?id=" + m.app.settings.shortAppName + "&platform=roku&format=xml&content-type=" + m.app.cp.contentType + "&video-fields=title&sdk=5.0"
		  getVideo: "http://api.adrise.tv/v2/video.php?app-id=" + m.app.settings.shortAppName + "&platform=roku&content-type=" + m.app.cp.contentType + "&format=xml&id="
		  getVideos: "http://api.adrise.tv/v2/videos.php?app-id=" + m.app.settings.shortAppName + "&platform=roku&content-type=" + m.app.cp.contentType + "&format=xml&id="
		}

'Commented out until CMS for Docurama goes live		
'if san = "docurama"
'	m.app.cp.urls = {
'		  getPlaylists: "http://cms.adrise.com/v2/app.php?id=" + m.app.settings.shortAppName + "&platform=roku&format=xml&content-type=" + m.app.cp.contentType + "&video-fields=title&sdk=5.0"
'		  getVideo: "http://cms.adrise.com/v2/video.php?app-id=" + m.app.settings.shortAppName + "&platform=roku&content-type=" + m.app.cp.contentType + "&format=xml&id="
'		  getVideos: "http://cms.adrise.com/v2/videos.php?app-id=" + m.app.settings.shortAppName + "&platform=roku&content-type=" + m.app.cp.contentType + "&format=xml&id="
'		}
'end if

m.app.player.pingFrequency = 360

m.app.player.temp = m.app.player.playVideo

m.app.player.playVideo  = function (episode)
    episode.SwitchingStrategy="full-adaptation"
    return m.temp(episode)
end function

m.app.utils.tempLE = m.app.utils.logEvent

m.app.utils.logEvent = function (evt)
  if evt.trackType <> "viewthru"
    m.tempLE(evt)
  end if
end function
...
if deviceInfo.GetDisplayType() <> "HDTV"
  m.app.player.pv = m.app.player.playVideo
...
if san = "threshold"
app.utils.showChangedAppMessage  = function (background, fontColor, loadingImageUrl, message)
  ...
  loadingText = {
    ...
    TargetRect: {
        ...
        y: loadingImage.TargetRect.y + 240
...
app.utils.showChangedAppMessage ("#4C0080", "#ffffff", app.settings.adrise_loadingurl, "Threshold has been changed to Premiere! Please click OK to view this channel now.")
end if

m.app.utils.getSubscription = function()
  g = GetGlobalAA()
  return g.app.player.subscription
end function
...
m.app.utils.cancelAsyncRequest = function(id) as Object
...
m.app.serverLinker = function(utils, player, settings)
    o =  {
      utils: utils
      settings: settings
      player: player
...
if san = "gravitas" or san = "godigital" or san = "audio-crush" or san = "indie-crush" or san = "midnight-pulp" or san = "all-fitness-tv"
m.app.gridScreen.app = m.app
...
'---------------------------------
m.app.gridScreen.showToolsRow = function (rokuGridScreen)
...
m.app.detailScreen.origUpdateButtons = m.app.detailScreen.updateButtons

'------------------------------------------------------------------
m.app.detailScreen.updateButtons = function(screen, episode, showRentButton)
