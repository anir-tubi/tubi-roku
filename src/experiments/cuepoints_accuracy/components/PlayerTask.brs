...
'     RAF.setTrackingCallback(logFunc, logObj)
    ...
    if content.midroll_position_integer = invalid and content.midroll_position_float = invalid
      ' only do this if VMAP is positioning ad breaks
      adPods = RAF.getAds()
      keepPlaying = RAF.showAds(adPods, invalid, view)
    end if

    port = CreateObject("roMessagePort")
    if content.notification_interval <> invalid and content.notification_interval > 0
      print "Setting notification interval to "; content.notification_interval
      video.notificationInterval = content.notification_interval
    else
      video.notificationInterval = 0.5
    end if
    if content.relay_position <> invalid and content.relay_position <> false
      m.top.observeField("position", port)
    else
            ...
            if msg.GetField() = "position" then
                ...
                if content.midroll_position_integer <> invalid and content.midroll_position_integer <> 0
                  ...
                  if curPos = content.midroll_position_integer
                    adPods = RAF.getAds()
                    content.midroll_position_integer = 0
                  end if
                ...
                ' Compare by float (might be before or after
                  ...
                  if curPos >= content.midroll_position_float
                    adPods = RAF.getAds()
                    content.midroll_position_float = 0.0
                  end if

                ' let RAF check.  Needs VMAP midroll positioning
                else 
                ...
                if adPods <> invalid and adPods.count() > 0
                    ...
                    video.visible = content.video_node_visible
                    video.control = "stop"  
                end if
