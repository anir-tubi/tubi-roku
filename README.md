...
16\. Email Roku Partner Success to let them know the build is in their queue.

17\. Push the tag corresponding to the build that was just pushed to Github `$ git push origin x_y_z`
...
At times we may want to override certain things in the application. These can be accomplished with similar steps to the proxy above. To remove all overrides run:
...
To disable magic link can use a request like. This is primarily for use by Roku `curl -d '' 'http://<ROKU-IP>:8060/launch/41468?password=499zsaHvENIYuEiVPMMa3S5w&setRegistry=%7B%22overrides%22%3A%20%7B%22externalConfig%22%3A%20%22%7B%5C%22auth_magic_link_enabled%5C%22%3A%20false%7D%22%7D%7D'`
