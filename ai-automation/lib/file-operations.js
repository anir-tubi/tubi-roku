...
function extractItBlock(generatedTest) {
  ...
  console.log(`ℹ️  Extracted it() block (${itBlockLines.length} lines): First line: ${itBlockLines[0].trim().substring(0, 80)}, Last line: ${itBlockLines[itBlockLines.length - 1].trim().substring(0, 80)}`);
...
function selectTargetDescribe(describeBlocks, testDetails) {
  ...
  console.log(`ℹ️  Looking for describe matching: mainSection="${testDetails.mainSection}", subSection="${testDetails.subSection}"`);
  ...
  if (!targetDescribe) {
    ...
    console.log(`ℹ️  ✅ Matched describe block: "${targetDescribe.name}" (lines ${targetDescribe.startLine + 1}-${targetDescribe.endLine + 1})`);
  }
...
function findCorrectInsertPosition(fileContent, testDetails) {
  ...
  console.log(`ℹ️  Found ${describeBlocks.length} describe blocks: ${describeBlocks.map(d => `"${d.name}" (lines ${d.startLine + 1}-${d.endLine + 1})`).join(', ')}`);

  // Sanity check for unclosed describe blocks
  if (unclosedDescribes.length > 0) {
    console.log(`⚠️  Found ${unclosedDescribes.length} unclosed describe block(s): ${unclosedDescribes.map(d => d.name).join(', ')}`);
  ...
  if (itBlocks.length > 0) {
    ...
    console.log(`ℹ️  ✅ Will insert after line ${lastIt.endLine + 1} in "${targetDescribe.name}"`);
    return insertPos;
  } else {
    console.log(`ℹ️  No it() blocks found in "${targetDescribe.name}", inserting before closing brace at line ${targetDescribe.endLine + 1}`);
...
function saveTest(generatedTest, testDetails) {
  ...
  if (fs.existsSync(filePath)) {
    console.log(`ℹ️  File exists: ${fileName}.ts - appending new test case`);

    const existingContent = fs.readFileSync(filePath, 'utf8');
    const extraction = extractItBlock(generatedTest);

    if (!extraction.success) {
      console.log(`⚠️  ${extraction.error}. Creating new file instead.`);
    ...
    console.log('ℹ️  ✅ Added .only to new test case for isolated execution');
    ...
    if (insertPosition === -1) {
      ...
      console.log('ℹ️  ✅ Added new test case to existing file with .only');
    }
  } else {
    console.log(`ℹ️  Creating new file: ${fileName}.ts`);
    fs.writeFileSync(filePath, generatedTest);
  }

  console.log(`ℹ️  📁 Test saved: ${filePath}`);
  return filePath;
}
