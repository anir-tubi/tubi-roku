...
function cleanGeneratedCode(stdout) {
  ...
  for (const pattern of explanationPatterns) {
    if (pattern.test(cleanedOutput)) {
      ...
      logger.success('Removed explanation phrase');
      break;
    }
  }
  ...
  if (importMatch && importMatch.index > 0) {
    ...
    logger.success('Removed explanatory text before imports');
  }
  ...
  if (firstImportLineIndex > 0) {
    ...
    logger.success('Removed additional explanatory text before imports');
  }
...
function addOnlyToSpecificTest(code, testCaseId = null) {
  ...
  if (testCaseId) {
    ...
    if (match) {
      ...
      logger.success(`Added .only to test ${normalizedId}`);
      return result;
    } else {
      logger.warning(`Could not find test with ID ${normalizedId}, adding .only to last test`);
    }
  }
  ...
  if (itBlocks && itBlocks.length > 0) {
    ...
    logger.success('Added .only to last test');
    return result;
  }

  logger.warning('Could not add .only - no it() blocks found');
  return code;
}
