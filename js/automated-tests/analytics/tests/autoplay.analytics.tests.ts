...
describe('Autoplay Analytics', function () {
	...
	it('Start video - From autoplay field when autoplay is automatic C130134 \
      and Play progress - From autoplay field when autoplay is automatic C130136 @analytics', async () => {
	...
	it('Autoplay - Action show "videoId" is correct for tv show C515573 and Page load - When next episode playback starts C130132 \
      ...
      and Autoplay - Series - Next episode plays after multiple consecutive autoplays C6030 @analytics', async () => {
	...
	it('UI: C21201 Autoplay - When user presses the Back button then series autoplay UI is dismissed @analytics', async () => {
		await testUtils.startApplicationAtPage('tv', {
			shouldCreateNewUser: false,
		});
		const homePage = HomePage();
	...
	it('UI: C21205 - When autoplay timer expires then next episode autoplays @analytics', async () => {
		await testUtils.startApplicationAtPage('tv', {
			shouldCreateNewUser: false,
		});
		const homePage = HomePage();
	...
	it('Autoplay - Action show is fired when autoplay dismissed and viewed again C515574 and \
      Autoplay - Action dismiss when autoplay is dismissed C515575 @analytics', async () => {
		await testUtils.startApplicationAtPage('tv', {
			shouldCreateNewUser: false,
		});
		const homePage = HomePage();
	...
	it('UI: C25123,C105693 - Autoplay - When user chooses last title on the list then movie plays @analytics', async () => {
		const homePage = HomePage();
		const titleId = await homePage.getMovieTitleId();
		const playback = await homePage.playMovieTitle();
		await playback.seekToAutoplay();
